...
public:
  ...
  Quat operator ^ (double& alpha) {
    ...
    double theta = atan2(sqrt(dot(q_,q_)-q_[0]*q_[0]), q_[0]);
    if (abs(sin(theta)) > CS175_EPS2)
      return Quat(cos(theta*alpha), Cvec3(q_[1],q_[2],q_[3])*sin(theta*alpha)/sin(theta));
    else 
      return Quat(cos(theta*alpha), Cvec3(q_[1],q_[2],q_[3]));
  }
